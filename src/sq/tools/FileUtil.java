...
//		cutGeneralFile("E://Difficult.java", "E://Cow//"); // 剪切文件
	 ...
	 * 读取JSON文件
	 */
	public static String readFileByJSON(String fileName) {
		...
		try {
			...
			reader = new BufferedReader(new InputStreamReader(fr, "GBK"));
			String tempString = null;
			// 一次读入一行，直到读入null为文件结束
			while ((tempString = reader.readLine()) != null) {
				retJson.append(tempString); 
			}
			reader.close();
		} catch (IOException e) {
			e.printStackTrace();
	 ...
	 * 写JSON文件
	 */
	public static boolean writeFileByJSON(String jsonStr,String filename) {
		FileWriter fw = null; 
		try { 
	 ...
	 * @return
	 */
	public static boolean writeFileByFileWriter(List<String> inList,String filename) { 
		FileWriter fw = null; 
		try { 
			fw = new FileWriter(filename);
			long begin3 = System.currentTimeMillis();
